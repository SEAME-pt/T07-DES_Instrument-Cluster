...
TEST_CASE("Testes completos da Classe ZMQReader", "[zmqreader]") {
    ...
    zmqReader.start();
    std::this_thread::sleep_for(std::chrono::milliseconds(200));  // Aumentado tempo de conexão
    ...
    SECTION("Teste de parada emergencial") {
        std::thread sender([&]() {
    ...
    // SECTION("Teste de reconexão") {
    //     zmqReader.stop();
    //     std::this_thread::sleep_for(std::chrono::milliseconds(500));
        
    //     // Reinicializa o socket e contexto
    //     zmqReader.start();
    //     std::this_thread::sleep_for(std::chrono::milliseconds(3000));  // Tempo maior para reconexão

    //     const std::string msg1 = "speed 150";
    //     publisher.send(zmq::buffer(msg1.data(), msg1.size()), zmq::send_flags::none);
    //     REQUIRE(spySpeed.wait(7000));  // Timeout aumentado
    //     if (spySpeed.count() > 0) {
    //         REQUIRE(spySpeed.takeFirst().at(0).toString() == "150");
    //     }
    // }
    ...
    // Espera para garantir o fechamento
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
}
